 ...
 * @deprecated
 */
public class LegacyCsvReader implements AutoCloseable {
     ...
     */
    public LegacyCsvReader(BufferedReader fileReader, Character delimiter, Character escapeChar) {
     ...
     * @throws LegacyBrokenCsvStructureException custom exception, see exceptions.BrokenCsvStrictureException
     * @throws IOException                 I\O Exception
     */
    public LinkedList<String> readLine() throws LegacyBrokenCsvStructureException, IOException {
     ...
     * @throws LegacyBrokenCsvStructureException Custom exception, represents the broken structure
     *                                     line which produced it and the character number in
     *                                     this line
     */
    private void processEscapeCharacter(int i, String line) throws LegacyBrokenCsvStructureException {
        if (!escapedField) {
     ...
     * @throws LegacyBrokenCsvStructureException Custom exception with info what line produced the fail, alas
     *                                     I have no Idea how to point the actual place in the string which is broken.
     */
    private void validateStructure(String line) throws LegacyBrokenCsvStructureException {
        if (symmetryCheck == 0) {
    ...
    private void throwException(String line, Integer charIndex, Cause cause) throws LegacyBrokenCsvStructureException {
        escapedField = false;
        stepForward = false;
        throw new LegacyBrokenCsvStructureException(line,
                charIndex,
                symmetryCheck, cells.size(),
                cells.toString(),
                cause);
